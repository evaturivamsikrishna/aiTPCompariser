"""
Offline judge: score generated test plans against a stock intent checklist.

Uses fuzzy title matching. Stock STR/expected results are ignored even if present.
No API keys.
"""
...
def compare_and_evaluate_plan(
    stock_plan: GeneratedTestPlan,
    candidate_plan: GeneratedTestPlan,
    source_intents: Optional[List[SourceIntent]] = None,
) -> ModelEvaluation:
    """
    Score a generated plan only. Stock titles are coverage intents.
    Matching is by title similarity, never by comparing STR/expected results.
    """
    stock_cases = stock_plan.test_cases
    gen_cases = candidate_plan.test_cases

    logging.info(
        "Evaluating generated plan '%s' against %s stock intents (%s generated cases).",
    ...
    if not stock_cases:
        return ModelEvaluation(
            ...
            error="Stock plan has no test cases to compare against.",
        )
    if not gen_cases:
        return ModelEvaluation(
            model_name=candidate_plan.model_name,
    ...
    stock_areas = sorted({(tc.area or "Unspecified").strip() or "Unspecified" for tc in stock_cases})
    ...
    for area in stock_areas:
        ...
        if best >= PARTIAL_MATCH:
            area_hits += 1
    area_ratio = area_hits / max(len(stock_areas), 1)
    area_metric = MetricScore(
        score=_ratio_to_score(area_ratio),
        reasoning=(
            f"{area_hits}/{len(stock_areas)} stock areas have generated cases "
            f"({', '.join(stock_areas[:8])}{'…' if len(stock_areas) > 8 else ''})."
        ),
    )

    avg_similarity = sum(best_similarities) / len(best_similarities)
    match_confidence_ratio = avg_similarity / 100.0
    ...
    scope_keep_ratio = 1.0 - drift_ratio
    ...
    avg_steps = (
        sum(len(tc.steps) for tc in gen_cases) / len(gen_cases) if gen_cases else 0.0
    )
    ...
    diagnostics = {
        ...
        "stock_intents": len(stock_cases),
        "unmatched_stock": len(stock_cases) - covered,
        "stock_areas": len(stock_areas),
        "areas_covered": area_hits,
        "avg_title_similarity_pct": round(avg_similarity, 1),
    ...
    evaluation = ModelEvaluation(
        ...
        requirement_coverage=MetricScore(
            score=coverage_score,
            reasoning=(
                f"{covered}/{len(stock_cases)} stock intents matched "
                f"({semantic} strong, {covered - semantic} partial). "
                "Matched by title similarity; stock STR/expected results are not required."
            ),
        ),
        match_confidence=MetricScore(
            score=_ratio_to_score(match_confidence_ratio),
            reasoning=(
                f"Average best title similarity across stock intents is {avg_similarity:.0f}%. "
                "High means generated titles clearly correspond to stock intents, not weak partials."
            ),
        ),
        ...
        edge_case_quality=MetricScore(
            ...
            reasoning=(
                f"{extra_count} extra in-scope cases beyond stock intents "
        ...
        scope_drift=MetricScore(
            score=_ratio_to_score(scope_keep_ratio),
            reasoning=(
                f"{len(extra_out_of_scope)}/{len(gen_cases)} generated cases are weakly related "
                f"to any stock intent (below {IN_SCOPE_EXTRA}% similarity)."
            ),
        ),
        traceability=MetricScore(
            score=_ratio_to_score(trace_ratio),
            reasoning=(
                f"{healthy_maps}/{len(alignments)} stock intents map to 1–2 generated cases. "
                f"{fan_in} generated cases cover 3+ stock intents; {oversplit} intents split across 5+ cases."
            ),
        ),
    )
    logging.info(
